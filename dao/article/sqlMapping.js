var sqlMapping = {
    // getCategoryListByParentId: 'SELECT a.catagoryId as id ,a.categoryName,a.createTime,a.updateTime FROM table_category a LEFT JOIN table_category b ON a.parentId = b.catagoryId WHERE a.parentId = ?',
    ...
    list: 'SELECT a.articleId,a.title,a.content,a.updateTime,a.createTime,a.authorUserId,b.userNickName,b.userAvatar,b.userGender,b.userPhone,b.userBirthday,c.categoryId,c.categoryName FROM table_article a LEFT JOIN table_user b ON (a.authorUserId=b.userId) LEFT JOIN table_category c ON (a.categoryId=c.categoryId) WHERE a.title LIKE CONCAT("%",?,"%") OR b.userNickName LIKE CONCAT("%",?,"%") OR c.categoryName LIKE CONCAT("%",?,"%") LIMIT ?,?',
    count: 'SELECT COUNT(*) FROM table_article a LEFT JOIN table_user b ON (a.authorUserId=b.userId) LEFT JOIN table_category c ON (a.categoryId=c.categoryId) WHERE a.title LIKE CONCAT("%",?,"%") OR b.userNickName LIKE CONCAT("%",?,"%") OR c.categoryName LIKE CONCAT("%",?,"%")',
